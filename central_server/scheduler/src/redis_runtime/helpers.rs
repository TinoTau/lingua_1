...
pub(crate) fn normalize_instance_id(s: &str) -> String {
...
fn parse_xpending_entries(value: redis::Value) -> Vec<PendingEntry> {
    ...
    for e in entries {
        ...
        if let (Some(id), Some(_consumer), Some(idle_ms), Some(deliveries)) = (id, consumer, idle_ms, deliveries) {
...
/// 将 Redis HGETALL 返回值转换为 HashMap<String, String>
/// 
/// Redis HGETALL 返回格式: Bulk([key1, value1, key2, value2, ...])
#[allow(dead_code)]  // 在测试代码中使用
pub(crate) fn redis_value_to_hashmap(value: redis::Value) -> Option<HashMap<String, String>> {
    // HashMap 已在父模块 redis_runtime.rs 中导入
    let redis::Value::Bulk(items) = value else { return None };
