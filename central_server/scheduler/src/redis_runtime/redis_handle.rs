impl RedisHandle {
    ...
    /// 此方法主要用于测试。在生产代码中，应通过 Phase2Runtime 来获取 RedisHandle。
        ...
        Ok(Self {
            ...
            min_ttl_seconds: scheduler_cfg.retry.min_ttl_seconds,
            min_ttl_ms: scheduler_cfg.retry.min_ttl_ms,
            stream_min_maxlen: scheduler_cfg.retry.redis_stream_min_maxlen,
        })
    }
    ...
    pub async fn set_ex_string(&self, key: &str, val: &str, ttl_seconds: u64) -> redis::RedisResult<()> {
        ...
        cmd.arg(key).arg(val).arg("EX").arg(ttl_seconds.max(self.min_ttl_seconds));
    ...
    /// DEL - 删除键
    pub async fn del(&self, key: &str) -> redis::RedisResult<u64> {
    ...
    async fn xadd_payload_maxlen(&self, stream: &str, payload: &str, maxlen: usize) -> redis::RedisResult<String> {
        ...
        cmd.arg(stream)
            .arg("MAXLEN")
            .arg("~")
            .arg(maxlen.max(self.stream_min_maxlen))
            .arg("*")
            .arg("payload")
            .arg(payload);
        self.query(cmd).await
    ...
    pub async fn hgetall(&self, key: &str) -> redis::RedisResult<std::collections::HashMap<String, String>> {
    ...
    /// HINCRBY - Hash 字段值增加指定数值
    pub async fn hincrby(&self, key: &str, field: &str, increment: i64) -> redis::RedisResult<i64> {
    ...
    /// SETEX - 设置键值并设置过期时间（秒）
    pub async fn setex(&self, key: &str, seconds: u64, value: &str) -> redis::RedisResult<String> {
    ...
    async fn set_nx_px(&self, key: &str, val: &str, ttl_ms: u64) -> redis::RedisResult<bool> {
        ...
        let mut cmd = redis::cmd("SET");
        cmd.arg(key).arg(val).arg("NX").arg("PX").arg(ttl_ms.max(self.min_ttl_ms));
        // OK => Some("OK")；失败 => Nil
