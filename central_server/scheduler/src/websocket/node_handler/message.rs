use super::util::extract_service_from_details;
use crate::app_state::AppState;
use crate::messages::{ErrorCode, NodeMessage, SessionMessage, UiEventStatus, UiEventType};
...
pub(super) async fn handle_node_message(
    ...
    match message {
        ...
        NodeMessage::JobResult {
            ...
            // Phase 1：收到结果即释放 reserved 并发槽（幂等）
            state.node_registry.release_job_slot(&nid, &job_id).await;

            // Phase 1：支持 failover 重派时，必须忽略“过期节点”的结果（避免竞态覆盖）
            let job = state.dispatcher.get_job(&job_id).await;
            if let Some(ref j) = job {
                if matches!(j.status, crate::dispatcher::JobStatus::Completed | crate::dispatcher::JobStatus::Failed) {
                    warn!(
                        trace_id = %trace_id,
                        job_id = %job_id,
                        node_id = %nid,
                ...
                if j.assigned_node_id.as_deref() != Some(&nid) {
                    warn!(
                        trace_id = %trace_id,
                        job_id = %job_id,
                        node_id = %nid,
            ...
            if success {
                ...
                if let Some(ref text_asr) = text_asr {
                    if !text_asr.is_empty() {
                        ...
                        state
                            .session_connections
                            .send(&session_id, Message::Text(ui_event_json))
                            .await;
                    }
                }
                    ...
                    if let Some(ref job_info) = job {
                        if let Some(target_session_ids) = &job_info.target_session_ids {
                            ...
                            if let Some(room_code) =
                                state.room_manager.find_room_by_session(&session_id).await
                            {
                            ...
                            for target_session_id in target_session_ids {
                                if !state
                                    .session_connections
                                    .send(target_session_id, Message::Text(result_json.clone()))
                                    .await
                                {
                                    warn!(
                                        trace_id = %trace_id,
                            ...
                            if !state
                                .session_connections
                                .send(&session_id, Message::Text(result_json))
                                .await
                            {
                ...
                if let Some(err) = job_error {
                    ...
                    state
                        .session_connections
                        .send(&session_id, Message::Text(error_json))
                        .await;
                }
            }
        }
        ...
        NodeMessage::AsrPartial {
            ...
            if !state
                .session_connections
                .send(&session_id, Message::Text(partial_json))
                .await
            {
