...
✅ **已优化**：将聚合和去重逻辑从 `PostProcessCoordinator` 迁移到 `PipelineOrchestrator`：

* **PipelineOrchestrator**：负责 ASR 任务编排（音频聚合 → ASR 识别 → 文本聚合 → 合并处理 → 文本过滤 → 语义修复 → 去重检查）
* **PostProcessCoordinator**：只负责翻译、TTS 和 TONE
...
ASR（语音识别）
  ↓
PipelineOrchestrator.AggregationStage（文本聚合）- 统一处理
  ↓
PipelineOrchestrator.MergeHandler（合并处理）
  ↓
PipelineOrchestrator.TextFilter（文本过滤）
  ↓
PipelineOrchestrator.SemanticRepairStage（语义修复）
  ↓
PipelineOrchestrator.DedupStage（去重）
  ↓
PostProcessCoordinator.TranslationStage（NMT）
  ↓
PostProcessCoordinator.TTSStage（TTS）
  ↓
PostProcessCoordinator.TONEStage（TONE）
```
...
- ✅ 避免了状态不一致风险
- ✅ 简化了流程，提高了性能
- ✅ 职责更清晰：PipelineOrchestrator 处理 ASR 相关，PostProcessCoordinator 处理翻译和音频生成
