...
- **处理流程**：
  ...
  8. 去重检查（DedupStage）
- **已优化**：
  - ✅ 聚合、合并、过滤、去重逻辑已迁移到 PipelineOrchestrator
  - ✅ 统一了 ASR 相关的处理流程
- **当前问题**：
  - ⚠️ 即使 `use_asr=false`，仍然会被调用（返回空结果）
...
4. PipelineOrchestrator.processJob()
   ...
   └─> 音频聚合（AudioAggregator）- 将多个短句聚合成完整句子
   └─> ASR 识别（ASRHandler）- 语音识别
   └─> ASR 结果处理（ASRResultProcessor）- 空文本检查、无意义文本检查
   └─> 文本聚合（AggregationStage）- 处理跨utterance的边界重复
   └─> 合并处理（MergeHandler）- 处理合并逻辑，取消被合并的 GPU 任务
   └─> 文本过滤（TextFilter）- 处理文本过滤逻辑（shouldDiscard、shouldWaitForMerge）
   └─> 内部重复检测（detectInternalRepetition）- 检测并移除文本内部重复
   └─> 语义修复（SemanticRepairStage）- 修复 ASR 识别错误
   └─> 去重检查（DedupStage）- 基于 job_id 去重（30秒 TTL）
   └─> 构建 JobResult（包含 should_send、dedup_reason）
   └─> 返回 JobResult
...
6. PostProcessCoordinator.process()
   └─> 检查 should_send（如果 false，直接返回空结果）
   └─> 检查文本是否为空（如果为空，返回空结果）
...
│                                                               │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐     │
│  │ ASR      │  │Aggregation│ │Merge/    │  │Semantic │     │
│  │ Stage    │→ │  Stage   │→ │Filter   │→ │ Repair  │     │
│  │          │  │          │  │  Stage  │  │  Stage  │     │
│  └──────────┘  └──────────┘  └──────────┘  └──────────┘     │
│                                                               │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐     │
│  │ Dedup    │  │Translation│ │   TTS   │  │   TONE  │     │
