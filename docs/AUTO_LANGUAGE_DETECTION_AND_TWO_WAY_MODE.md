...
1. ✅ 创建 `LanguageDetector` 模块（`node-inference/src/language_detector.rs`）
2. ✅ 扩展消息协议（`SessionInit`、`InferenceRequest`、`JobAssign` 等）
3. ✅ 修改推理流程（支持 `src_lang="auto"`）
4. ✅ 扩展调度服务器和客户端消息类型
5. ✅ 实现实际的语言检测逻辑（使用 Whisper + 文本特征推断）✅
...
1. ✅ 实现实际的语言检测逻辑（使用 Whisper + 文本特征推断）✅
2. ✅ ASR 引擎共享 Whisper 上下文给 LanguageDetector ✅
3. ✅ 错误处理完善 ✅
4. ✅ 单元测试（7个测试，全部通过）✅
5. ⏸️ 性能优化（语言检测缓存）
6. ⏸️ UI 优化（语言检测结果显示）
...
**总工作量**：3-4 周（核心功能 2 周已完成，剩余 1-2 周用于 UI 和集成测试）
