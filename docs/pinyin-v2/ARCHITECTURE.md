# Pinyin-IME-V2 & FW Detector 主链 — V2.0 架构（已冻结）

**版本**：Pinyin-IME-V2 **V2.0** · FW Mainline **V2**  
**冻结日期**：2026-06-03  
**代码**：`main/src/fw-detector/pinyin-ime-v2/` · `fw-detector-orchestrator.ts`
...
| 项 | 结论 |
|----|------|
| IME 职责 | **Span Discovery** — 定位 raw 文本高风险区间 |
| 4A–4D | **已实现并冻结** |
| 主链 | 下文 §3 **唯一允许路径** |
| IME 性能 | decode avg **~8ms**，非 pipeline 瓶颈 |
| 端到端 Apply | **未验收**（KenLM 0 approved）— **不阻塞 IME 冻结** |
| 下一阶段 | **KenLM Audit**（禁止 IME 功能扩展） |

**允许**：Bug Fix、Regression、Freeze Contract、文档更新。  
**禁止**：架构回滚、Direct Repair、绕过 Recall/KenLM、新 Span 来源、TopK>5、Lattice/Backpointer/Graph Search。
...
rawAsrText
  → Pinyin-IME-V2
  → HintGate (runPinyinImeV2HintGate)
  → Recall (recallSpanTopK)
  → Candidate Builder
  → KenLM
...
**编排**：`runFwDetectorOrchestrator` → **仅** `resolvePinyinImeV2Spans`（非 `resolveFwSpans`、非活动路径 `suspicious-span-detector-v1`）。

**早退**：无 approved span → `segmentForJobResult = rawText`。
...
| — | HintGate | `pinyin-ime-v2-hint-gate.ts` | 见 §7 |
...
## 7. HintGate 与 Proposal 接线

| 步骤 | 函数 | 输出 |
|------|------|------|
| Proposal | `runPinyinImeV2SpanProposal` | `PinyinImeV2SpanProposal` |
| Gate | `runPinyinImeV2HintGate` | `PinyinImeV2ApprovedSpan[]` |
| Map | `mapApprovedSpansToFwSpans` | `FwSpanDiagnostics[]` |
| Resolve | `resolvePinyinImeV2Spans` | spans + `pinyinImeV2` diagnostics |

**Normalizer**：`normalizePinyinImeV2Spans` 合并 diff + instability + `boundaryCompatibleTopKSpans`；音节/字数门控。

**邻居门控**：`lexiconNearNeighbor` → `recallSpanTopK`（Recall 层，非 IME）。
...
静态断言摘要：无 `segmentForJobResult` / 无 `pinyin-ime-v1` / 无 `backpointer`；orchestrator 含 `resolvePinyinImeV2Spans`；含 `buildBoundaryCompatibleTopKDiff`。
...
Dialog200 限时批测（89 条，2026-06-03）：

| 指标 | 4B.1 前 | 4D 后 |
|------|---------|--------|
| diffZeroBoundaryPositive | 0 | **61** |
| boundary spans (proposal) | 0 | **190** |
| approved cases | 5/110 | **29/89** |
| Recall 有候选 span | — | **49/49** |
| KenLM approved | 0 | **0** |
| Apply | 0 | **0** |

**IME 判定**：Span Discovery **达成**；端到端修复 **未达成**（下游责任）。
...
| Span 过多 | HintGate 门控；宁可漏报 |

原则：**宁可漏报，不允许误修**（IME 不写回）。
...
| 1 | IME | `runPinyinImeV2SpanProposal` / `resolvePinyinImeV2Spans` |
| 2 | HintGate | `runPinyinImeV2HintGate` |
| 3–6 | FW | `runFwSentenceRerankPipeline` |
...
| FW 模块 README | `main/src/fw-detector/README.md`（实现细节以代码为准；Span 源为 IME） |
