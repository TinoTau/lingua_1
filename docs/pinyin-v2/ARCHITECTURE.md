...
rawAsrText
  → Pinyin-IME-V2
  → SpanSelector (selectPinyinImeV2Spans)
...
**早退**：无 selected span（`skippedReason=no_selected_spans`）→ `segmentForJobResult = rawText`。
...
| 2 | SpanSelector | `selectPinyinImeV2Spans` |
