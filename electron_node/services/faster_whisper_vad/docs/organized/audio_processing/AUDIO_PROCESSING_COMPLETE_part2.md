...
MIN_AUDIO_DURATION = 0.5  # 最短音频时�?.5�?
```

**日志显示**�?
- 音频时长：`4.56秒` �?足够�?
- `condition_on_previous_text=False` �?已生�?
- 但识别结果仍然很�?

**可能的原�?*�?
1. **音频被截�?*：VAD过早停止发送，导致音频不完�?
...
DEFAULT_SILENCE_FILTER_CONFIG: {
  ...
  windowMs: 100,
  attackFrames: 3,
  releaseFrames: 30, // �?5增加�?0�?00ms，允许更长的停顿�?
}
```

**理由**�?
- 150ms的停顿太短，用户在说话过程中经常会有200-300ms的停�?
- 增加�?00ms可以避免过早截断
...
DEFAULT_SILENCE_FILTER_CONFIG: {
  ...
  threshold: 0.015,
  attackThreshold: 0.015,
  releaseThreshold: 0.005, // �?.008降低�?.005（更宽松，避免误停止�?
  windowMs: 100,
  attackFrames: 3,
  releaseFrames: 30,
}
...
**理由**�?
- 降低releaseThreshold可以让VAD在更低的音量下继续发�?
...
3. **上下文参�?*：检查`initial_prompt`是否正确传�?
...
2. 测试场景�?
   ...
   - **期望**：VAD不会�?50ms时停止，应该继续发送直�?00ms静音
...
- `electron_node/services/faster_whisper_vad/docs/ISSUE_STATUS_REPORT.md` - 问题状态报�?
...
**关键操作**:
- `audioBuffer.push(audioData)` - 累积音频�?
- `audioBuffer.splice(0, 10)` - **移除并返回前10�?*（关键！�?
- `audioBuffer = []` - 清空所有剩余帧
...
T=0.0s-0.1s: 收到10�?
  ...
  �?audioBuffer.splice(0, 10) �?移除�?0帧，返回[frame0...frame9]
  �?sendAudioChunk([frame0...frame9], false)
  �?audioBuffer = [] �?已清空，不会重复发�?

T=0.1s-0.2s: 收到10�?
  �?audioBuffer.push(frame10, frame11, ..., frame19)
  �?audioBuffer.length = 10
  �?audioBuffer.splice(0, 10) �?移除�?0帧，返回[frame10...frame19]
...
T=2.9s-3.0s: 收到10�?
  ...
  �?audioBuffer.length = 10
  �?audioBuffer.splice(0, 10) �?移除�?0帧，返回[frame290...frame299]
  �?sendAudioChunk([frame290...frame299], false)
  �?audioBuffer = [] �?已清�?

T=3.0s: 用户点击发送按�?
  �?sendCurrentUtterance()
  �?audioBuffer.length = 0 (因为已经全部通过audio_chunk发�?
  �?跳过发送（audioBuffer为空�?
```
...
```
T=0.0s-2.4s: �?00ms发送一次audio_chunk
  �?已发�? frame0-frame239 (24次，每次10�?
  �?audioBuffer = [] (每次发送后都清�?
...
**关键�?*:
- `splice(0, 10)` **移除**�?0帧，不会重复发�?
- 剩余的帧继续留在buffer中，等待下次发送或手动发�?
...
**关键�?*:
- 只发送buffer�?*剩余的数�?*（还没有通过audio_chunk发送的�?
- 发送后立即清空buffer，避免重�?

### 3. onSilenceDetected() - 静音检测后发�?
...
// chunk = [frame0, frame1, ..., frame9] �?已发�?
// audioBuffer = [frame10, frame11] �?剩余数据，未发�?
...
```
录音2.5秒，用户在第2.5秒点击发�?
  �?T=0.0s-2.4s: �?00ms发送audio_chunk (frame0-239)
  �?T=2.4s-2.5s: 收到frame240-249，但还没�?00ms
  �?T=2.5s: 用户点击发�?
    �?sendUtterance(frame240-249) �?只发送剩余数�?
  �?audioBuffer = [] �?清空
```
...
// take_combined会移除buffer，避免重�?
```

**关键�?*:
- 每个`audio_chunk`消息都会累积到同一个`utterance_index`的buffer
- `take_combined()`�?*移除**buffer，避免重复使�?
...
**场景**:
1. 用户说话，通过`audio_chunk`发送部分数�?
2. 用户点击发送按钮，通过`utterance`发送剩余数�?
3. 调度服务器可能收到两个数据源
...
1. **积累**: `audioBuffer.push(audioData)` - 持续累积音频�?
2. **自动发�?*: �?00ms，`splice(0, 10)`移除�?0帧并发�?
3. **剩余数据**: 保留在buffer中，等待下次发送或手动发�?

### 与utterance的错开

1. **不会重复**: `splice()`会移除已发送的数据
2. **utterance只发送剩�?*: `sendCurrentUtterance()`只发送buffer中剩余的数据
3. **清空机制**: 发送后立即清空buffer，避免重�?
...
### 0.24秒音�?= 只收到了2-3个audio_chunk

**计算**:
- 0.24�?= 240ms
- 每个audio_chunk = 100ms�?0�?× 10ms/帧）
- 0.24�?�?2-3个audio_chunk
...
**场景**:
- Web端录音开�?
- 发送了2-3个audio_chunk�?.2-0.3秒）
- Web端静音检测触�?�?停止发送audio_chunk
- 调度服务器等�?秒后超时 �?finalize �?只有0.24秒音�?
...
#### 原因3: 调度服务器finalize机制有问�?⚠️

**场景**:
- Web端正常发送audio_chunk
- 但调度服务器的finalize机制过早触发
- 只累积了2-3个chunk就finalize�?
