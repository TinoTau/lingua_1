# 设计缺陷分析：Pending音频处理问题

**日期**: 2026-01-28  
**结论**: **主要是设计缺陷**，而不是代码逻辑问题
...
**设计意图**（用户澄清）：
- ✅ 用户的长语音在调度服务器生成多个job
- ✅ 这些job以maxDuration finalize的方式发送给节点端
- ✅ 最后一个job一定是以手动或timeout finalize收尾的
- ✅ pendingMaxDurationAudio只需要等待最后一个手动或timeout finalize出现即可
- ✅ 不需要TTL
