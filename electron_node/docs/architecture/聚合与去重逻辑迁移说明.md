...
PipelineOrchestrator.processJob():
  1. 检查 use_asr（如果 false，返回空结果）
  2. 音频聚合（AudioAggregator）
     └─ 8秒阈值判断，延迟3秒等待合并
  3. ASR 识别（ASRHandler）
     └─ TaskRouter.routeASRTask()
  4. ASR 结果处理（ASRResultProcessor）
     ├─ 空文本检查
     └─ 无意义文本检查
  5. 文本聚合（AggregationStage）
  ...
  8. 内部重复检测（detectInternalRepetition）
     └─ 检测并移除文本内部重复
  9. 语义修复（SemanticRepairStage）
     └─ TaskRouter.routeSemanticRepairTask()
  10. 去重检查（DedupStage）
      └─ 基于 job_id，30秒 TTL
  11. 构建 JobResult（包含 should_send 和 dedup_reason）
  12. 返回 JobResult

PostProcessCoordinator.process():
  1. 检查 should_send（如果 false，直接返回）
  2. 检查文本是否为空（如果为空，返回空结果）
  3. 翻译（TranslationStage）
     ├─ 检查 use_nmt
