...
**注意**: 流程说明仍然有效，但以下内容已过期：
- 第273行：`shouldCommit` 字段已移除
- 第436行：`AggregatorResult` 接口中的 `shouldCommit` 字段已移除
...
3. **获取聚合后的文本**:
   ```typescript
   // ⚠️ 已更新：shouldCommit 已移除，现在使用 isLastInMergedGroup 判断
   if (aggregatorResult.isLastInMergedGroup && aggregatorResult.text) {
...
- `TTSResult`: tts_audio
- `AggregatorResult`: action, text, metrics, isLastInMergedGroup (⚠️ shouldCommit 已移除)
