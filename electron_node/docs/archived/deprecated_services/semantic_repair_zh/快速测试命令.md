# Semantic Repair 中文语义修复服务 - 快速测试命令
...
$result = Invoke-RestMethod -Uri "http://127.0.0.1:5013/repair" -Method Post -Body $body -ContentType "application/json"

Write-Host "输入: $($body | ConvertFrom-Json | Select-Object -ExpandProperty text_in)" -ForegroundColor Cyan
Write-Host "输出: $($result.text_out)" -ForegroundColor Green
Write-Host "决策: $($result.decision)" -ForegroundColor Yellow
Write-Host "置信度: $($result.confidence)" -ForegroundColor Yellow
Write-Host "耗时: $($result.repair_time_ms) ms" -ForegroundColor Gray
```
...
```json
{
  ...
  "diff": [],
  "reason_codes": [],
  "repair_time_ms": 200-500
}
```

## 性能指标（GPU 模式）

- **响应时间**: 200-500 ms（之前 CPU 模式是 ~4000ms）
- **CPU 使用率**: 低（大部分计算在 GPU 上）
- **GPU 使用率**: 高（推理时）

## 注意事项

1. 服务启动需要 10-30 秒（模型加载和预热）
2. 首次请求可能稍慢（模型预热）
3. 确保服务已完全启动后再测试
