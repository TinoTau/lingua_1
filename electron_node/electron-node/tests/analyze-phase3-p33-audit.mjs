#!/usr/bin/env node
/**
 * P3.3 FW Metadata Span Gate — quality + perf analysis.
 */
import fs from 'fs';
...
const reportPath = path.join(__dirname, 'lexicon-v2-phase3-p33-batch-result.json');
...
const phase3P32 = loadJson('lexicon-v2-phase3-p32-quality-perf.json');
...
const out = {
  ...
  recall_chain: {
    path: 'FW Metadata Span Gate → V2 Recall (LIMIT 2/3/0) → KenLM weak_veto → pick',
  },
  span_gate: {
    span_count_per_job: stats(spanCounts),
    fw_metadata_gate_ms: stats(metaGateMs),
    kenlm_span_gate_query_count: { count: 0, avg: 0, p50: 0, p95: 0, p99: 0, min: 0, max: 0 },
  },
  recall_tier: {
  ...
  comparison: {
    phase2: phase2
      ...
      : null,
    phase3_p32: phase3P32
      ? {
          fw_applied_total: phase3P32.contract?.fw_applied_total,
          avg_cer_final: phase3P32.quality?.avg_cer_final,
          pipeline_ms_p95: phase3P32.perf?.pipeline_total_ms?.p95,
          kenlm_span_gate_ms_avg: phase3P32.perf?.kenlm_span_gate_ms?.avg,
        }
      : null,
  },
};

const outPath = path.join(__dirname, 'lexicon-v2-phase3-p33-quality-perf.json');
