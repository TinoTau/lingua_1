#!/usr/bin/env node
/**
 * Phase 2 dialog_200 batch: FW pipeline + Lexicon V2 Intent enabled.
 * After batch, drain wait + export-all for lexiconSessionIntent stats.
 *
 *   set PROJECT_ROOT=D:\Programs\github\lingua_1
 *   node tests/run-lexicon-v2-phase2-dialog200-batch.js
 *   node tests/run-lexicon-v2-phase2-dialog200-batch.js --limit 20
 */
const fs = require('fs');
...
async function exportAllSessions(port) {
  const res = await fetch(`http://127.0.0.1:${port}/session-migration/export-all`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ sourceNodeId: 'phase2-dialog200' }),
...
function summarizeIntentSessions(sessions) {
  const p2 = sessions.filter((s) => String(s.sessionId || '').startsWith('p2-d200-'));
  let withIntent = 0;
  let withKeywords = 0;
  ...
  return {
    ...
    write_rate: p2.length > 0 ? withIntent / p2.length : 0,
  };
}
...
async function main() {
  ...
  const report = {
    ...
    testScope: 'Lexicon V2 Phase 2 dialog_200',
    port,
  ...
  for (const caseDef of cases) {
    ...
    const sessionId = `p2-d200-${caseDef.id}-${Date.now()}`;
    try {
  ...
  const outPath = path.join(__dirname, 'lexicon-v2-phase2-dialog200-batch-result.json');
