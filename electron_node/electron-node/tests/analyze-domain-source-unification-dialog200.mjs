...
const batchPath = path.join(__dirname, 'domain-source-unification-dialog200-batch-result.json');
...
const recallSources = {};
const domainVotes = {};
const hierarchyVersions = new Set();
...
for (const c of cases) {
  ...
  const hv = runtime.domainHierarchyVersion;
  if (hv) hierarchyVersions.add(hv);
  ...
  if (sampleIds.includes(c.id)) {
    samples.push({
      ...
      recallFineDomains: (runtime.recallDomainScope ?? v4.recallEnabledFineDomains ?? []).slice(0, 8),
      availableFineDomains: (runtime.availableFineDomains ?? []).length,
      llmAllowedDomains: runtime.llmAllowedDomains ?? [],
      domainHierarchyVersion: hv ?? null,
...
const out = {
  ...
  domain_unification: {
    ...
    domainHierarchyVersions: [...hierarchyVersions],
    lexicon_runtime_ok: batch.summary.lexicon_runtime_ok_count,
    fw_triggered: batch.summary.fw_triggered_count,
    fw_applied_cases: batch.summary.fw_applied_case_count,
  },
  samples,
};

const outPath = path.join(__dirname, 'domain-source-unification-dialog200-quality-perf.json');
