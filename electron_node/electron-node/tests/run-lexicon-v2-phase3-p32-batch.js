...
async function main() {
  ...
  const port = getPort();
  console.log('P3.2 KenLM Span Gate batch — test server', port);
  ...
  const report = {
    ...
    testScope: 'P3.2 KenLM Span Gate + V2 Recall (Phase 3 Only)',
    config: {
  ...
  for (const caseDef of cases) {
    ...
    const sessionId = `p32-d200-${caseDef.id}-${Date.now()}`;
    try {
  ...
  const outPath = path.join(__dirname, 'lexicon-v2-phase3-p32-batch-result.json');
