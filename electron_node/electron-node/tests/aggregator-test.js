...
const aggregator_1 = require("../main/src/aggregator");
/**
 * 测试 1: 基本 merge 决策
 */
function testBasicMerge() {
    console.log('\n=== 测试 1: 基本 merge 决策 ===');
    const manager = new aggregator_1.AggregatorManager();
    const sessionId = 'test-session-1';
    // 第一个 utterance
    const segments1 = [
        { text: '我们今天讨论一下', start: 0, end: 1.0 }
    ];
    const result1 = manager.processUtterance(sessionId, '我们今天讨论一下', segments1, { top1: 'zh', p1: 0.92, top2: 'en', p2: 0.04 }, 0.8, true, false, 'offline');
    ...
    const result2 = manager.processUtterance(sessionId, '这个方案', segments2, { top1: 'zh', p1: 0.9, top2: 'en', p2: 0.05 }, 0.7, true, false, 'offline');
...
function testHardGap() {
    ...
    manager.processUtterance(sessionId, '我们先到这里。', segments1, { top1: 'zh', p1: 0.95, top2: 'en', p2: 0.02 }, 0.9, true, false, 'offline');
    // 第二个 utterance（长 gap，应该 new_stream）
    const segments2 = [
        { text: '下一件事', start: 4.0, end: 4.5 }
    ];
    const result2 = manager.processUtterance(sessionId, '下一件事', segments2, { top1: 'zh', p1: 0.93, top2: 'en', p2: 0.03 }, 0.8, true, false, 'offline');
    console.log('第二个 utterance 结果:', {
        action: result2.action,
        shouldCommit: result2.shouldCommit,
        text: result2.text,
...
function testDedup() {
    ...
    manager.processUtterance(sessionId, '我们', segments1, { top1: 'zh', p1: 0.9 }, 0.8, true, false, 'offline');
    // 第二个 utterance（有重复）
    const segments2 = [
        { text: '我们可以', start: 0.6, end: 1.1 }
    ];
    const result2 = manager.processUtterance(sessionId, '我们可以', segments2, { top1: 'zh', p1: 0.9 }, 0.8, true, false, 'offline');
    console.log('第二个 utterance 结果:', {
        action: result2.action,
        shouldCommit: result2.shouldCommit,
        text: result2.text,
...
function testLangSwitchNotConfident() {
    ...
    manager.processUtterance(sessionId, '我们用 OpenAI', segments1, { top1: 'zh', p1: 0.78, top2: 'en', p2: 0.18 }, 0.6, true, false, 'offline');
    // 第二个 utterance（英文，但置信度不高，应该 merge）
    const segments2 = [
        { text: 'API 来做', start: 1.4, end: 1.9 }
    ];
    const result2 = manager.processUtterance(sessionId, 'API 来做', segments2, { top1: 'en', p1: 0.74, top2: 'zh', p2: 0.22 }, 0.6, true, false, 'offline');
    console.log('第二个 utterance 结果:', {
        action: result2.action,
        shouldCommit: result2.shouldCommit,
        text: result2.text,
...
function testFlush() {
    ...
    manager.processUtterance(sessionId, '第一句话', segments1, { top1: 'zh', p1: 0.9 }, 0.8, true, false, 'offline');
    const segments2 = [
        { text: '第二句话', start: 0.6, end: 1.1 }
    ];
    manager.processUtterance(sessionId, '第二句话', segments2, { top1: 'zh', p1: 0.9 }, 0.8, true, false, 'offline');
    // Flush
