#!/usr/bin/env node
/**
 * P4 Sentence-Level Rerank + Tone Pinyin batch — V2 Recall ON, Metadata Gate.
 */
const fs = require('fs');
...
async function main() {
  ...
  const port = getPort();
  console.log('P4 Sentence Rerank batch — test server', port);
  ...
  for (const caseDef of cases) {
    ...
    const sessionId = `p4-d200-${caseDef.id}-${Date.now()}`;
    try {
      ...
      if (row.pass) {
        pass += 1;
        const sr = row.sentence_rerank;
        console.log(
          `[${caseDef.id}] PASS pipeline_ms=${row.pipeline_ms} spans=${row.span_count} apply=${row.fw_applied_count} pickedRaw=${sr?.pickedIsRaw ?? 'n/a'}`
        );
      } else {
        fail += 1;
  ...
  const outPath = path.join(__dirname, 'lexicon-v2-p4-batch-result.json');
