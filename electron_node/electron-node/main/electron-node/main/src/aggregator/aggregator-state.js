...
class AggregatorState {
    ...
    processUtterance(text, segments, langProbs, qualityScore, isFinal = false, isManualCut = false, isPauseTriggered = false, isTimeoutTriggered = false) {
        ...
        let action;
        if (this.lastUtterance && (this.lastUtterance.isManualCut || this.lastUtterance.isPauseTriggered)) {
            ...
            // 正常决策
            action = (0, aggregator_decision_1.decideStreamAction)(this.lastUtterance, curr, this.mode, this.tuning);
        }
        // 更新指标
        if (action === 'MERGE') {
        ...
        if (action === 'MERGE' && this.lastUtterance) {
            ...
            if (this.tailBuffer) {
                const tailDedup = (0, dedup_1.dedupMergePrecise)(this.tailBuffer, text, this.dedupConfig);
            ...
            if (this.pendingText) {
                // 提交时移除 tail
                const textToCommit = (0, tail_carry_1.removeTail)(this.pendingText, this.tailCarryConfig);
                const tail = (0, tail_carry_1.extractTail)(this.pendingText, this.tailCarryConfig);
                if (tail) {
        ...
        if (shouldCommitByManualCut && action === 'MERGE') {
            ...
            shouldCommitNow = (0, aggregator_decision_1.shouldCommit)(this.pendingText, this.lastCommitTsMs, nowMs, this.mode, this.tuning) || shouldCommitByManualCut || shouldCommitByTimeout || isFinal;
        ...
        if (shouldCommitNow && this.pendingText) {
            ...
            if (isFinal || isManualCut) {
                ...
                // 非 final，保留 tail
                commitText = (0, tail_carry_1.removeTail)(this.pendingText, this.tailCarryConfig);
                const tail = (0, tail_carry_1.extractTail)(this.pendingText, this.tailCarryConfig);
                if (tail) {
                    this.tailBuffer = tail;
                    this.metrics.tailCarryUsage++;
