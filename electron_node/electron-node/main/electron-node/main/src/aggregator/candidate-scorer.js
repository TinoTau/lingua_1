 ...
 * 对候选翻译进行打分
 */
function scoreCandidates(candidates, originalText, originalTranslation, previousTranslation, config = {}) {
    const finalConfig = { ...DEFAULT_CONFIG, ...config };
    const scored = candidates.map(({ candidate, translation }) => {
        // 规则分（40%）
        const ruleScore = calculateRuleScore(candidate, originalText, previousTranslation);
        // NMT 分（40%）
        const nmtScore = calculateNMTScore(translation, originalTranslation, previousTranslation);
        // 语言模型分（20%）
        const lmScore = calculateLMScore(candidate, originalText);
        // 综合得分
        const totalScore = ruleScore * finalConfig.ruleWeight +
 ...
 * - 长度惩罚：过短或过长的候选减分
 */
function calculateRuleScore(candidate, originalText, previousTranslation) {
    let score = 0.5; // 基础分
    // 1. 数字保护（20%）
    ...
    if (originalNumbers.length > 0) {
        const numberMatch = originalNumbers.every((num, idx) => candidateNumbers[idx] === num);
        if (numberMatch) {
            score += 0.2;
        }
        else {
 ...
 */
function calculateNMTScore(translation, originalTranslation, previousTranslation) {
    let score = 0.5; // 基础分
    // 1. 翻译自然度（40%）
 ...
 * - 语法正确性：语法是否正确（简化处理）
 */
function calculateLMScore(candidate, originalText) {
    let score = 0.5; // 基础分
    // 1. 文本自然度（60%）
    // 简化为：检查是否包含明显的错误模式
 ...
 * 检查翻译是否包含不自然的模式
 */
function checkUnnaturalPatterns(translation) {
    // 检查重复的单词或短语
 ...
 * 计算翻译一致性（与上下文的连贯性）
 */
function calculateConsistency(translation, previousTranslation) {
    // 简化为：检查是否有共同的词汇或短语
 ...
 * 选择最佳候选（考虑最小分数提升阈值）
 */
function selectBestCandidate(scoredCandidates, originalTranslation, minScoreImprovement = 0.05) {
    if (scoredCandidates.length === 0) {
        return null;
    }
    // 找到得分最高的候选
    const best = scoredCandidates.reduce((a, b) => a.score > b.score ? a : b);
    // 计算原始翻译的得分（作为基准）
