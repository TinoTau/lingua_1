...
exports.detectBadSegment = detectBadSegment;
 ...
 * @returns 坏段检测结果
 */
function detectBadSegment(asrResult, audioDurationMs, previousText) {
    const reasonCodes = [];
    let qualityScore = 1.0;
    // 如果没有 segments 信息，无法进行检测
    if (!asrResult.segments || asrResult.segments.length === 0) {
        // 如果没有 segments，但音频时长很长，可能是异常（统一使用20字符标准）
        if (audioDurationMs && audioDurationMs > 2000 && asrResult.text.trim().length < 20) {
    ...
    for (let i = 1; i < segments.length; i++) {
        ...
        if (prevSegment.end !== undefined && prevSegment.end !== null &&
            ...
            if (!isNaN(gap) && isFinite(gap) && gap > 1.0) {
                ...
                logger_1.default.debug(`Bad segment detected: large gap between segments ${i - 1} and ${i}: ${gap.toFixed(2)}s`);
            }
        }
    }
    ...
    if (audioDurationMs !== undefined) {
        ...
        if (avgSegmentDuration > 5.0) {
            ...
            logger_1.default.debug(`Bad segment detected: average segment duration too long: ${avgSegmentDuration.toFixed(2)}s`);
        }
        // 音频时长 >= 1.5 秒但 segments 数 <= 1，可能是异常
        if (audioDurationSec >= 1.5 && segmentCount <= 1) {
            reasonCodes.push('LONG_AUDIO_FEW_SEGMENTS');
            qualityScore = Math.max(0.0, qualityScore - 0.3);
            logger_1.default.debug(`Bad segment detected: long audio (${audioDurationSec.toFixed(2)}s) but few segments (${segmentCount})`);
        }
    }
    // 3. 检测平均 segment 时长异常（过短或过长）
    if (segments.length > 0) {
        const segmentsWithTimestamps = segments.filter(seg => seg.start !== undefined && seg.start !== null &&
            seg.end !== undefined && seg.end !== null);
        if (segmentsWithTimestamps.length > 0) {
            const totalDuration = segmentsWithTimestamps.reduce((sum, seg) => {
            ...
            if (!isNaN(avgDuration) && isFinite(avgDuration) && avgDuration < 0.1) {
                ...
                qualityScore = Math.max(0.0, qualityScore - 0.2);
                logger_1.default.debug(`Bad segment detected: average segment duration too short: ${avgDuration.toFixed(2)}s`);
            }
        }
    }
        ...
        if (firstSegment.start !== undefined && firstSegment.start !== null &&
            ...
            if (audioDurationMs !== undefined && !isNaN(segmentsCoverage) && isFinite(segmentsCoverage)) {
                ...
                if (!isNaN(coverageRatio) && isFinite(coverageRatio) && coverageRatio < 0.5) {
                    ...
                    qualityScore = Math.max(0.0, qualityScore - 0.2);
                    logger_1.default.debug(`Bad segment detected: segments coverage (${segmentsCoverage.toFixed(2)}s) is only ${(coverageRatio * 100).toFixed(0)}% of audio duration (${audioDurationSec.toFixed(2)}s)`);
                }
            }
        }
    }
    ...
    if (garbageRatio > 0.1) {
        ...
        qualityScore = Math.max(0.0, qualityScore - 0.3);
        logger_1.default.debug(`RERUN-1: High garbage ratio detected: ${(garbageRatio * 100).toFixed(1)}%`);
    }
    ...
    if (previousText && previousText.trim().length > 0) {
        ...
        if (overlap > 0.8) {
            ...
            qualityScore = Math.max(0.0, qualityScore - 0.3);
            logger_1.default.debug(`RERUN-1: High overlap with previous text detected: ${(overlap * 100).toFixed(1)}%`);
        }
    }
    // 判断是否为坏段
 ...
 * @returns 乱码字符数量
 */
function countGarbageChars(text) {
    let garbageCount = 0;
