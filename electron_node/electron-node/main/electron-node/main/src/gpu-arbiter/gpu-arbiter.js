...
class GpuArbiter {
    constructor(config) {
        // 每个GPU的互斥锁和队列
        this.mutexes = new Map(); // gpuKey -> isLocked
        this.queues = new Map(); // gpuKey -> queue
        this.activeLeases = new Map(); // leaseId -> lease
        // 指标统计
    ...
    async acquire(request) {
        ...
        if (!this.mutexes.has(gpuKey)) {
            logger_1.default.error({ gpuKey, taskType }, 'GpuArbiter: Invalid GPU key');
            return {
                status: "SKIPPED",
                reason: "GPU_BUSY",
            };
