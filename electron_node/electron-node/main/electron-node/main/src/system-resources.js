...
async function getGpuUsage() {
    ...
    logger_1.default.debug({}, 'Starting to fetch GPU usage');
    // 使用 Promise.race 确保不会超过超时时间
    const timeoutPromise = new Promise((resolve) => {
        setTimeout(() => {
            logger_1.default.debug({}, 'GPU usage fetch timeout, returning null');
            resolve(null);
        }, GPU_FETCH_TIMEOUT);
    });
    const fetchPromise = (async () => {
        ...
        try {
            logger_1.default.debug({}, 'Attempting to fetch GPU info via nvidia-smi');
            const result = await getGpuUsageViaNvidiaSmi();
            if (result) {
                logger_1.default.debug({ result }, 'Successfully fetched GPU info via nvidia-smi');
                return result;
            }
        }
        catch (error) {
            logger_1.default.debug({ error }, 'nvidia-smi method failed, trying alternative');
        }
        ...
        try {
            ...
            if (result) {
                logger_1.default.debug({ result }, 'Successfully fetched GPU info via Python pynvml');
                return result;
            }
        }
        catch (error) {
            logger_1.default.debug({ error }, 'Python pynvml method failed');
        }
        logger_1.default.debug({}, 'All GPU info fetch methods failed, GPU info will not be displayed');
        return null;
    })();
...
async function getGpuUsageViaNvidiaSmi() {
    return new Promise((resolve) => {
        ...
        const timeout = setTimeout(() => {
            nvidiaSmi.kill();
            logger_1.default.debug({}, 'nvidia-smi command timeout, killed process');
            resolve(null);
        }, 1500);
        let output = '';
        let errorOutput = '';
        ...
        nvidiaSmi.on('error', (error) => {
            ...
            logger_1.default.debug({ error: error.message }, 'nvidia-smi command execution error (command may not exist)');
            resolve(null);
        });
    });
}
...
`;
        ...
        const tryNextPython = () => {
            ...
            timeout = setTimeout(() => {
                python.kill();
                logger_1.default.debug({ command: pythonCommands[currentIndex] }, 'Python GPU script timeout, killed process');
                currentIndex++;
                tryNextPython();
            }, 1000);
            let output = '';
            ...
            python.on('close', (code) => {
                ...
                if (code === 0 && output.trim() !== 'ERROR') {
                    try {
                        ...
                        logger_1.default.debug({ parseError, output }, 'Failed to parse Python output');
                    }
                }
                // 当前命令失败，尝试下一个
                currentIndex++;
