...
exports.detectASRLanguages = detectASRLanguages;
/**
 * 检测 ASR 服务的语言
 */
async function detectASRLanguages(service, models, metadataManager) {
    const languages = [];
    ...
    if (languages.length === 0) {
        ...
        const whisperMeta = allMetadata.find(m => m.model_id.includes('whisper') || m.model_id.includes('faster-whisper'));
        if (whisperMeta) {
