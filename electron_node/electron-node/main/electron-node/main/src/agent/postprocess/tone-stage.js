...
class TONEStage {
     ...
     */
    async process(job, ttsAudio, ttsFormat, speakerId) {
        const startTime = Date.now();
        // 检查是否需要生成 TONE
        if (job.pipeline?.use_tone !== true) {
            logger_1.default.debug({ jobId: job.job_id, sessionId: job.session_id }, 'TONEStage: TONE disabled by pipeline config, skipping TONE');
            return {
                toneAudio: undefined,
        ...
        if (!ttsAudio || ttsAudio.trim().length === 0) {
            logger_1.default.debug({ jobId: job.job_id, sessionId: job.session_id }, 'TONEStage: TTS audio is empty, skipping TONE');
            return {
                toneAudio: undefined,
                toneFormat: undefined,
                speakerId: undefined,
        ...
        if (!this.taskRouter) {
            logger_1.default.error({ jobId: job.job_id, sessionId: job.session_id }, 'TONEStage: TaskRouter not available');
            return {
                toneAudio: undefined,
                toneFormat: undefined,
                speakerId: undefined,
        ...
        const finalSpeakerId = speakerId || job.speaker_id || job.voice_id;
        if (!finalSpeakerId) {
            logger_1.default.warn({ jobId: job.job_id, sessionId: job.session_id }, 'TONEStage: Missing speaker_id, skipping TONE');
            return {
                toneAudio: undefined,
                toneFormat: undefined,
                speakerId: undefined,
        ...
        try {
            ...
            const executor = (0, sequential_executor_factory_1.getSequentialExecutor)();
            const taskType = 'TTS'; // TONE使用TTS的SequentialExecutor队列
            const sessionId = job.session_id || job.job_id;
            const utteranceIndex = job.utterance_index || 0;
            const toneResult = await executor.execute(sessionId, utteranceIndex, taskType, async () => {
                // GPU仲裁：获取GPU租约
                return await (0, gpu_arbiter_1.withGpuLease)('OTHER', // TONE不在GpuTaskType中，使用OTHER
                async () => {
                    const toneTask = {
                        audio: ttsAudio,
