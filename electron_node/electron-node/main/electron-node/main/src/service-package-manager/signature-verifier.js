...
function loadPublicKeys() {
    ...
    if (envKeys) {
        try {
            ...
            logger_1.default.error({ error }, 'Failed to parse public keys from environment variable');
        }
    }
...
class SignatureVerifier {
        ...
        for (const keyConfig of keyConfigs) {
            try {
                ...
                if (publicKeyBuffer.length !== 32) {
                    logger_1.default.warn({ key_id: keyConfig.key_id, length: publicKeyBuffer.length }, 'Invalid Ed25519 public key length (expected 32 bytes)');
                    continue;
                }
                this.publicKeys.set(keyConfig.key_id, publicKeyBuffer);
                logger_1.default.info({ key_id: keyConfig.key_id }, 'Public key loaded');
            }
            catch (error) {
                logger_1.default.error({ error, key_id: keyConfig.key_id }, 'Failed to load public key');
            }
        }
        if (this.publicKeys.size === 0) {
            logger_1.default.warn({}, 'No public keys loaded, signature verification will fail');
        }
    }
    /**
     * 验证服务包签名
     ...
     * @returns 验证是否通过
     */
    async verifySignature(variant, fileHash) {
        // 如果没有签名信息，返回 false（要求签名）
        if (!variant.signature) {
            logger_1.default.warn({ service_id: variant.artifact.url }, 'Service package has no signature');
            return false;
        }
        const signature = variant.signature;
        // 验证算法
        if (signature.alg !== 'ed25519') {
            logger_1.default.error({ algorithm: signature.alg }, 'Unsupported signature algorithm');
            return false;
        }
        // 获取公钥
        const publicKey = this.publicKeys.get(signature.key_id);
        if (!publicKey) {
            logger_1.default.error({ key_id: signature.key_id }, 'Public key not found for key_id');
            return false;
        }
        ...
        if (payload.sha256 !== fileHash) {
            logger_1.default.error({ expected: payload.sha256, actual: fileHash }, 'SHA256 hash mismatch in signed payload');
            return false;
        }
        ...
        try {
            ...
            logger_1.default.error({ error }, 'Failed to decode signature (base64)');
            return false;
        }
        // 验证签名长度（Ed25519 签名应该是 64 字节）
        if (signatureBuffer.length !== 64) {
            logger_1.default.error({ length: signatureBuffer.length }, 'Invalid Ed25519 signature length (expected 64 bytes)');
            return false;
        }
        ...
        try {
            ...
            if (publicKey.length === 0 || publicKey.toString('hex') === '00'.repeat(32)) {
                logger_1.default.warn({ key_id: signature.key_id }, 'Using placeholder public key, skipping signature verification');
            ...
            try {
                ...
                logger_1.default.debug({ error: nativeError.message }, 'Node.js native Ed25519 not available, trying alternative method');
                // 方法2：使用 tweetnacl（如果已安装）
                try {
                    // 动态导入 tweetnacl（可选依赖）
                    const nacl = require('tweetnacl');
                    // tweetnacl 的签名验证
                    const isValid = nacl.sign.detached.verify(payloadBuffer, signatureBuffer, publicKey);
                    if (isValid) {
                        logger_1.default.debug({ key_id: signature.key_id }, 'Signature verification passed (tweetnacl)');
                        return true;
                    }
                }
                catch (tweetnaclError) {
                    logger_1.default.warn({ error: tweetnaclError.message }, 'tweetnacl not available, signature verification skipped');
                    // 如果 tweetnacl 也不可用，在开发环境中允许通过
                    // 生产环境应该要求 tweetnacl 或 Node.js 15+
                    if (process.env.NODE_ENV === 'development' || !process.env.NODE_ENV) {
                        logger_1.default.warn({}, 'Development mode: signature verification skipped');
                        return true;
                    }
            ...
            // 如果所有方法都失败
            logger_1.default.error({ key_id: signature.key_id }, 'Signature verification failed');
            return false;
        }
        catch (error) {
            logger_1.default.error({ error, key_id: signature.key_id }, 'Failed to verify signature');
            return false;
        }
    }
    /**
     * 记录安全事件（签名验证失败）
     */
    logSecurityEvent(variant, reason, details) {
 ...
 * 创建签名验证器实例（单例）
 */
let signatureVerifierInstance = null;
