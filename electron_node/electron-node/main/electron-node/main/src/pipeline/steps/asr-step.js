...
const original_job_result_dispatcher_1 = require("../../pipeline-orchestrator/original-job-result-dispatcher");
const gpu_arbiter_1 = require("../../gpu-arbiter");
const job_pipeline_1 = require("../job-pipeline");
const logger_1 = __importDefault(require("../../logger"));
// 全局OriginalJobResultDispatcher实例（单例）
let originalJobResultDispatcher = null;
function getOriginalJobResultDispatcher() {
    if (!originalJobResultDispatcher) {
        originalJobResultDispatcher = new original_job_result_dispatcher_1.OriginalJobResultDispatcher();
    }
...
async function runAsrStep(job, ctx, services, options) {
    ...
    if (originalJobIds.length > 0) {
        ...
        for (const originalJobId of uniqueOriginalJobIds) {
            ...
            dispatcher.registerOriginalJob(job.session_id, originalJobId, expectedSegmentCount, originalJob, async (asrData, originalJobMsg) => {
                ...
                const originalCtx = (0, job_context_1.initJobContext)(originalJobMsg);
                ...
                if (originalJobMsg.lang_a && originalJobMsg.lang_b) {
                    const detectedLang = determineDetectedLanguage(asrData.languageProbabilities, originalJobMsg.lang_a, originalJobMsg.lang_b);
                    if (detectedLang) {
                ...
                try {
                    ...
                    if (services.resultSender) {
                        ...
                        services.resultSender.sendJobResult(originalJobMsg, result, startTime, result.should_send ?? true, result.dedup_reason);
                        logger_1.default.info({
                            originalJobId,
                            sessionId: job.session_id,
                    ...
                    logger_1.default.info({ originalJobId, sessionId: job.session_id }, 'runAsrStep: Original job pipeline completed');
                }
                catch (error) {
    ...
    for (let i = 0; i < audioSegments.length; i++) {
        ...
        try {
            ...
            if (originalJobIds.length > 0 && i < originalJobIds.length) {
                ...
                if (i === 0 && asrResult.shouldResetContext && services.sessionContextManager) {
    ...
    if (originalJobIds.length > 0 && !originalJobIds.includes(job.job_id)) {
        ...
        ctx.consolidatedToJobIds = Array.from(new Set(originalJobIds));
        logger_1.default.info({
            jobId: job.job_id,
