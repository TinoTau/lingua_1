    ...
    for (const cudaPath of cudaPaths) {
        if (fs.existsSync(cudaPath)) {
                    ...
                    for (const dll of cudnnDlls) {
                        ...
                        if (fs.existsSync(dllPath)) {
                            if (!foundCudnnPaths.includes(cudnnPath)) {
                                ...
                                logger_1.default.info({ cudnnPath, dll }, 'Found cuDNN DLL');
                            }
                            break; // 找到一个 DLL 就足够了
                        }
                    }
            ...
            if (foundCudnnPaths.length > 0) {
                ...
                logger_1.default.warn({ cudaPath }, 'CUDA environment configured but cuDNN not found. ONNX Runtime CUDA may not work.');
            }
            break;
        }
    }
