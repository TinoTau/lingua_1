...
describe('AudioAggregator 优化功能测试', () => {
  ...
  afterEach(() => {
    aggregator.clearBufferByKey('job-1');
    aggregator.clearBufferByKey('job-2');
    aggregator.clearBufferByKey('job-3');
  });
  ...
  describe('场景3：超长音频（MaxDuration finalize）', () => {
    it('MaxDuration 只追加不输出：应返回 shouldReturnEmpty，buffer 保留累积音频', async () => {
      ...
      expect(status?.totalDurationMs ?? 0).toBeGreaterThan(0);
    });
  });

  describe('bufferKey 稳定性', () => {
    it('同一 job 的 bufferKey 稳定，不同 job 的 bufferKey 不同', () => {
      const audio1 = createMockPcm16Audio(2000);
      const job1 = createJobAssignMessage('job-1', 'test-session-1', 0, audio1);
      const bufferKey1 = buildBufferKey(job1);
      expect(bufferKey1).toBe('job-1');
      expect(buildBufferKey(job1)).toBe(bufferKey1);

      const audio2 = createMockPcm16Audio(2000);
      const job2 = createJobAssignMessage('job-2', 'test-session-1', 1, audio2);
      const bufferKey2 = buildBufferKey(job2);
      expect(bufferKey2).toBe('job-2');
      expect(bufferKey1).not.toBe(bufferKey2);
    });
  });
});
