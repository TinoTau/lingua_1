...
export interface QueueManagerCallbacks {
  ...
  recordMetric: (gpuKey: string, metricName: string, value: number | string, increment?: number) => void;
  getAdmissionState: (gpuKey: string) => GpuAdmissionState | undefined;
  getGpuUsageFromCache: (gpuKey: string) => { usagePercent: number; sampledAt: number } | null;
  processQueueCallback?: (gpuKey: string) => void;
}

export class GpuArbiterQueueManager {
  private queues: Map<string, PendingRequest[]>;
   ...
   * 获取队列
   */
  getQueue(gpuKey: string): PendingRequest[] {
  ...
  processQueue(
    ...
    const result = this.callbacks.acquireImmediately(
      gpuKey,
      request.taskType,
      request.holdMaxMs,
      request.trace
   ...
   * 获取队列快照
   */
  getQueueSnapshot(gpuKey: string): Array<{
    leaseId: string;
    taskType: GpuTaskType;
    priority: number;
    waitTimeMs: number;
