...
export interface TranslationStageConfig {
  ...
  enableAsyncRetranslation?: boolean;
  asyncRetranslationThreshold?: number;
  nmtRepairEnabled?: boolean;
  nmtRepairNumCandidates?: number;
  nmtRepairThreshold?: number;
}
...
export class TranslationStage {
   ...
   */
  private cleanupExpiredPendingTranslations(): void {
  ...
  async process(
    ...
    if (!job.src_lang || !job.tgt_lang) {
      logger.warn(
        { jobId: job.job_id, srcLang: job.src_lang, tgtLang: job.tgt_lang },
        'TranslationStage: Missing language info, skipping translation'
      );
      return {
        translatedText: '',
        translationTimeMs: Date.now() - startTime,
    ...
    if (!this.taskRouter) {
      logger.error(
        ...
        'TranslationStage: TaskRouter not available'
      );
      return {
        translatedText: '',
        translationTimeMs: Date.now() - startTime,
    ...
    if (sourceCandidates.length === 1 && shouldRepair) {
      ...
      const scoredCandidates = scoreCandidates(
        ...
        aggregatedText,
        translatedCandidates[0]?.translation || '',
        previousTranslation
      );
