...
export class LanguageCapabilityDetector {
  ...
  async detectLanguageCapabilities(
    ...
    capabilities.supported_language_pairs = this.computeLanguagePairs(
   ...
   * 检测 ASR 服务的语言
   */
  private async detectASRLanguages(
    ...
    if (languages.length === 0) {
      ...
      const whisperMeta = this.modelMetadata.find(m => 
   ...
   * 检测 TTS 服务的语言
   */
  private async detectTTSLanguages(
    service: InstalledService,
    models: InstalledModel[]
  ): Promise<string[]> {
    const languages: string[] = [];
   ...
   * 检测语义修复服务的语言
   */
  private async detectSemanticLanguages(
    service: InstalledService,
    models: InstalledModel[]
  ): Promise<string[]> {
    const languages: string[] = [];
   ...
   * 查找模型元数据
   */
  private findModelMetadata(
    modelId: string | undefined,
    modelType: 'asr' | 'nmt' | 'tts' | 'semantic'
  ): ModelLanguageMetadata | undefined {
    if (!modelId) return undefined;

    const modelIdLower = modelId.toLowerCase();

    // 精确匹配
    let meta = this.modelMetadata.find(m => 
      m.model_id === modelId && m.model_type === modelType
    );

    // 模糊匹配（包含关键词）
    if (!meta) {
      meta = this.modelMetadata.find(m => {
        if (m.model_type !== modelType) return false;
    ...
    if (!meta) {
      if (modelType === 'nmt') {
        ...
        if (modelIdLower.includes('piper')) {
          ...
          meta = this.modelMetadata.find(m => 
            m.model_type === 'tts' && m.model_id.includes('piper')
          );
        }
