...
import {
  ...
  buildAsrRepairLifecycleFromCtx,
  buildCtcContract,
...
describe('legacy recover only — legacy-asr-repair-contract', () => {
  const baseJob = {
    job_id: 'j1',
    session_id: 's1',
    utterance_index: 0,
  ...
  it('always emits lexicon_runtime_status and asr_repair_contract_version', () => {
    ...
    expect(result.extra?.asr_repair_contract_version).toBe(LEGACY_ASR_REPAIR_CONTRACT_V5);
    expect(result.extra?.lexicon_runtime_status).toBe('ok');
    expect(result.extra?.asr_repair_lifecycle?.executed).toBe(true);
    expect(result.extra?.recover_contract_version).toBeUndefined();
    expect(result.extra?.recover_lifecycle).toBeUndefined();
  ...
  it('recover lifecycle skipped when lexicon gated off', () => {
    ...
    const lifecycle = buildAsrRepairLifecycleFromCtx(job, {}, 'job_use_lexicon_false');
  ...
  it('sentence_repair separates executed and modified', () => {
    const ctx: JobContext = {
      ...
      translatedText: 'seg',
      lexiconRuntimeStatus: 'ok',
      asrRepairLifecycle: { executed: true, gated: false, skipped: true, skipReason: 'no_window_expansion_candidate' },
      asrRepairLifecycleSkipReason: 'no_window_expansion_candidate',
    };
    const contract = buildLegacyAsrRepairContractExtra(baseJob, ctx);
