...
function makeUtteranceInfo(text: string, startMs: number, endMs: number): UtteranceInfo {
  return { text, startMs, endMs, lang: { top1: 'zh', p1: 1 } };
}
...
describe('AggregatorStateTextProcessor', () => {
  ...
  const tailCarryConfig: TailCarryConfig = DEFAULT_TAIL_CARRY_CONFIG;
  ...
  describe('v3 改造：不再输出空字符串作为丢弃信号', () => {
    ...
    describe('lastTail 分支', () => {
      it('完全包含时应该保留原文，不返回空字符串', () => {
        const lastUtterance = makeUtteranceInfo('这是一句完整的话，包含了很多内容', 0, 2000);
      ...
      it('短句（<=20字符）去重后为空时应该保留原文', () => {
        const lastUtterance = makeUtteranceInfo('这是第一句话', 0, 1000);
        const text = '第一句话';  // 短句，去重后可能为空
