 ...
 */
export async function runYourTtsStep(
  ...
  if (job.pipeline?.use_tone !== true) {
    logger.debug(
      ...
      'runYourTtsStep: TONE disabled, skipping YourTTS'
    );
    ctx.ttsAudio = '';
    ctx.ttsFormat = 'opus';
    return;
  ...
  if (!services.taskRouter) {
    logger.error(
      { jobId: job.job_id },
      'runYourTtsStep: TaskRouter not available'
    );
    ctx.ttsAudio = '';
    ctx.ttsFormat = 'opus';
    return;
  ...
  try {
    // 1. 选择 YourTTS 服务端点
    const endpoint = services.taskRouter.selectServiceEndpoint(ServiceType.TTS);
    if (!endpoint) {
      logger.warn(
        { jobId: job.job_id },
        'runYourTtsStep: No YourTTS service available, skipping'
      );
      ctx.ttsAudio = '';
      ctx.ttsFormat = 'opus';
      return;
    ...
    const audioData = response.data.audio as number[];
 ...
 */
function convertPcm16ToF32(pcm16Buffer: Buffer): number[] {
 ...
 * 将 f32 数组转换为 WAV Buffer
 */
function convertFloat32ArrayToWav(audioData: number[], sampleRate: number): Buffer {
  // 将 f32 数组转换为 int16
