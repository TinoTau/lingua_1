...
import { NeedRescoreDetector } from './need-rescore';
import { Rescorer } from './rescorer';
import { CandidateProvider } from './candidate-provider';
import { AggregatorState } from '../aggregator/aggregator-state';
...
export function runAcceptanceTests(): {
  ...
  try {
    const detector = new NeedRescoreDetector();
  ...
  try {
    const state = new AggregatorState('test-session', 'offline');
    state.processUtterance(
      '测试文本',
      undefined,
      { top1: 'zh', p1: 0.9 },
      0.8,
    ...
    const recentText = (state as any).getRecentCommittedText();
 ...
 * 打印验收测试结果
 */
export function printAcceptanceResults(): void {
