...
Write-Host "----------------------------------------" -ForegroundColor Gray
$result = Test-Endpoint -Name "Cluster Stats" -Url "$BASE_URL/api/cluster"
...
Write-Host "----------------------------------------" -ForegroundColor Gray
$result = Test-Endpoint -Name "Metrics" -Url "$BASE_URL/metrics"
if ($result.Success -and $result.Data) {
    $PassCount++
    $metrics = $result.Data
...
Write-Host "----------------------------------------" -ForegroundColor Gray
$result = Test-Endpoint -Name "Prometheus" -Url "$BASE_URL/prometheus"
if ($result.Success) {
    $PassCount++
...
Write-Host "----------------------------------------" -ForegroundColor Gray
$simUrl = "$BASE_URL/api/phase3/simulate?required=Asr&required=Nmt&src_lang=zh&tgt_lang=en"
...
if ($FailCount -eq 0) {
    ...
    Write-Host "  3. Monitor cluster at http://localhost:5010/api/cluster" -ForegroundColor Gray
    exit 0
} else {
