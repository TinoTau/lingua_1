...
Write-Host "Job 处理过程详细分析" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
foreach ($utteranceIndex in $UtteranceIndices) {
    ...
    $relevantLogs = $nodeContent | Select-String -Pattern "`"utteranceIndex`":$utteranceIndex[^0-9]" | Select-Object -Last 100
    ...
    # 1. BufferKey 检查（最关键）
    Write-Host "[1. BufferKey 检查]" -ForegroundColor Green
    $bufferKeyLogs = $relevantLogs | Select-String -Pattern "BufferKey|bufferKey"
    if ($bufferKeyLogs) {
        $bufferKeyLogs | ForEach-Object {
            $logLine = $_.Line
            if ($logLine -match '"bufferKey":"([^"]+)"') {
                Write-Host "  BufferKey: $($matches[1])" -ForegroundColor White
            }
        }
    } else {
        Write-Host "  未找到 BufferKey 日志" -ForegroundColor Red
    }
    Write-Host ""
    
    # 2. AudioAggregator 处理
    Write-Host "[2. AudioAggregator 处理]" -ForegroundColor Green
    ...
    # 3. Buffer 删除原因
    Write-Host "[3. Buffer 删除原因]" -ForegroundColor Green
    $bufferDeleteLogs = $relevantLogs | Select-String -Pattern "BufferDelete|decisionBranch"
    if ($bufferDeleteLogs) {
        $bufferDeleteLogs | ForEach-Object {
            $logLine = $_.Line
            if ($logLine -match '"decisionBranch":"([^"]+)"') {
                Write-Host "  判定分支: $($matches[1])" -ForegroundColor White
            }
            if ($logLine -match '"pendingTimeoutAudioLength":(\d+)') {
                Write-Host "  pendingTimeoutAudio 长度: $($matches[1]) bytes" -ForegroundColor White
            }
            if ($logLine -match '"pendingSmallSegmentsCount":(\d+)') {
                Write-Host "  pendingSmallSegments 数量: $($matches[1])" -ForegroundColor White
            }
            if ($logLine -match '"reason":"([^"]+)"') {
                Write-Host "  删除原因: $($matches[1])" -ForegroundColor White
            }
        }
    } else {
        Write-Host "  未找到 Buffer 删除日志" -ForegroundColor Gray
    }
    Write-Host ""
    
    # 4. 音频质量检查
    Write-Host "[4. 音频质量检查]" -ForegroundColor Green
    ...
    # 5. ASR 服务处理
    Write-Host "[5. ASR 服务处理]" -ForegroundColor Green
    ...
    if ($asrInputLogs) {
        Write-Host "  [ASR 输入]" -ForegroundColor Cyan
        $asrInputLogs | ForEach-Object {
            $logLine = $_.Line
            if ($logLine -match '"audioSizeBytes":(\d+)') {
                Write-Host "    音频大小: $($matches[1]) bytes" -ForegroundColor White
            }
            if ($logLine -match '"audioDurationMs":(\d+)') {
                Write-Host "    音频时长: $($matches[1])ms" -ForegroundColor White
            }
        }
    }
    
    if ($asrOutputLogs) {
        Write-Host "  [ASR 输出]" -ForegroundColor Cyan
        $asrOutputLogs | ForEach-Object {
            $logLine = $_.Line
            if ($logLine -match '"asrText":"([^"]+)"') {
                $asrText = $matches[1]
                Write-Host "    识别文本: $asrText" -ForegroundColor White
            }
            if ($logLine -match '"asrTextLength":(\d+)') {
                Write-Host "    文本长度: $($matches[1]) 字符" -ForegroundColor White
            }
    ...
    # 6. Aggregation 处理
    Write-Host "[6. Aggregation 处理]" -ForegroundColor Green
    ...
    # 7. NMT 服务处理
    Write-Host "[7. NMT 服务处理]" -ForegroundColor Green
    ...
    if ($nmtInputLogs) {
        Write-Host "  [NMT 输入]" -ForegroundColor Cyan
    ...
    if ($nmtOutputLogs) {
        Write-Host "  [NMT 输出]" -ForegroundColor Cyan
    ...
    # 8. 最终结果
    Write-Host "[8. 最终结果]" -ForegroundColor Green
