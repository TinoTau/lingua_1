...
// 注意：这个文件需要编译后才能运行
// 使用编译后的 JS 路径
const ServicePackageManager = require('../../main/service-package-manager').ServicePackageManager;
const ServiceRuntimeManager = require('../../main/service-runtime-manager').ServiceRuntimeManager;
const ServiceRegistryManager = require('../../main/service-registry').ServiceRegistryManager;
...
async function main() {
  ...
  // 清理测试目录（可选）
  // await fs.rm(SERVICES_DIR, { recursive: true, force: true }).catch(() => {});
