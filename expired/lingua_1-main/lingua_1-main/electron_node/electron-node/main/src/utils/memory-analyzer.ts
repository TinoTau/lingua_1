...
export class MemoryAnalyzer {
   ...
   * 估算字符串占用的内存（字节）
   */
  private static estimateStringMemory(str: string): number {
   ...
   */
  static analyzeAggregatorManager(manager: AggregatorManager | null): {
    totalSessions: number;
    activeSessions: number;
    estimatedMemoryMB: number;
  } | null {
   ...
   */
  static analyzeTranslationCache(cache: any): {
    size: number;
    maxSize: number;
    estimatedMemoryMB: number;
  } | null {
   ...
   * 分析 DedupStage 的内存占用
   */
  static analyzeDedupStage(lastSentTextMap: Map<string, string>): {
    lastSentTextCount: number;
    estimatedMemoryMB: number;
  } | null {
    if (!lastSentTextMap) {
  ...
  static analyzeMemory(
    ...
    dedupLastSentText: Map<string, string> | null
  ): MemoryUsageInfo {
    const result: MemoryUsageInfo = {
   ...
   * 记录内存使用情况到日志
   */
  static logMemoryUsage(info: MemoryUsageInfo): void {
    logger.info(
      {
