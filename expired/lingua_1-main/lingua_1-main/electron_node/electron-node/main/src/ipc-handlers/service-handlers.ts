...
import type { ServicePackageManager } from '../service-package-manager';
import type { RustServiceManager } from '../rust-service-manager';
import type { PythonServiceManager } from '../python-service-manager';
import {
  getCachedAvailableServices,
  getCachedServiceRanking,
  getLastCacheUpdate,
  getCacheTTL,
  setCachedAvailableServices,
  setCachedServiceRanking,
  setLastCacheUpdate,
} from './service-cache';
...
export function registerServiceHandlers(
  ...
  ipcMain.handle('get-installed-services', async () => {
    try {
      if (!serviceRegistryManager) {
        logger.warn({}, 'Service registry manager not initialized');
        return [];
      }
      ...
      logger.error({ error: error.message, stack: error.stack }, 'Failed to get installed services');
      return [];
    }
  });
  ...
  ipcMain.handle('get-available-services', async () => {
    ...
    if (cachedServices !== null && (now - lastUpdate) < ttl) {
      logger.debug({ serviceCount: cachedServices.length }, 'Returning cached available services');
      return [...cachedServices]; // 返回副本，避免外部修改
    }

    // 如果有旧缓存，先返回旧缓存，然后在后台更新（不阻塞界面）
    if (cachedServices !== null && cachedServices.length > 0) {
      logger.debug({ serviceCount: cachedServices.length }, 'Returning stale cache for available services, will refresh in background');
      // 在后台异步更新，不等待结果
      setImmediate(async () => {
        try {
          await refreshAvailableServices();
        } catch (error) {
          logger.debug({ error }, 'Background available services refresh failed, will retry on next request');
        }
      });
    ...
    try {
      const response = await Promise.race([
        ...
        new Promise<any[]>((resolve) => {
          setTimeout(() => {
            logger.debug({}, 'Available services fetch timeout, returning empty array');
            resolve([]);
          }, 2000); // 2秒超时，快速返回
        })
      ]);
      ...
      // 如果超时或返回空，返回空数组
      logger.debug({}, 'Available services fetch returned empty or timed out');
      return [];
    } catch (error: any) {
      logger.debug({ error: error.message }, 'Available services fetch failed, returning empty array');
      return [];
    }
  });

  // 辅助函数：刷新可用服务列表
  async function refreshAvailableServices(): Promise<any[]> {
    try {
      ...
      logger.debug({ statsUrl }, 'Fetching available services from scheduler');

      const response = await axios.get(statsUrl, {
        timeout: 2000, // 2秒超时，确保快速返回
        validateStatus: (status: number) => status < 500, // 接受 4xx 错误，但不接受 5xx
      });
      ...
      logger.debug({ serviceCount: services.length, statsUrl }, 'Successfully fetched available services from scheduler');
      return services;
    } catch (error: any) {
      // 记录错误但不抛出，让调用者处理
      logger.debug({
  ...
  ipcMain.handle('download-service', async (_, serviceId: string, version?: string, platform?: string) => {
    try {
      if (!servicePackageManager) {
        logger.error({}, 'Service package manager not initialized');
        throw new Error('服务包管理器未初始化');
      }

      logger.info({ serviceId, version, platform }, 'Starting service download');
      ...
      logger.info({ serviceId, version, platform }, 'Service downloaded and installed successfully');
      return true;
    } catch (error: any) {
      logger.error({ error: error.message, serviceId, version, platform }, 'Failed to download service');
  ...
  ipcMain.handle('uninstall-service', async (_, serviceId: string, version?: string) => {
    return await uninstallService(serviceId, version, serviceRegistryManager, rustServiceManager, pythonServiceManager);
  });

  ipcMain.handle('get-service-ranking', async () => {
    // 如果缓存有效且不为空，直接返回缓存数据（快速返回，不阻塞）
    const now = Date.now();
    const cachedRanking = getCachedServiceRanking();
    const lastUpdate = getLastCacheUpdate();
    const ttl = getCacheTTL();
    // 只有当缓存不为空且有效时才使用缓存
    if (cachedRanking !== null && cachedRanking.length > 0 && (now - lastUpdate) < ttl) {
      logger.debug({ rankingCount: cachedRanking.length }, 'Returning cached service ranking');
      return [...cachedRanking]; // 返回副本，避免外部修改
    }

    // 如果有旧缓存（即使为空），先返回旧缓存，然后在后台更新（不阻塞界面）
    if (cachedRanking !== null && cachedRanking.length > 0) {
      logger.debug({ rankingCount: cachedRanking.length }, 'Returning stale cache for ranking, will refresh in background');
      // 在后台异步更新，不等待结果
      setImmediate(async () => {
        try {
          await refreshServiceRanking();
        } catch (error) {
          logger.debug({ error }, 'Background ranking refresh failed, will retry on next request');
        }
      });
    ...
    // 如果缓存为空，强制刷新（不返回空缓存）
    if (cachedRanking !== null && cachedRanking.length === 0) {
      logger.info({}, 'Cache is empty, forcing refresh for service ranking');
    }
    ...
    try {
      const response = await Promise.race([
        ...
        new Promise<any[]>((resolve) => {
          setTimeout(() => {
            logger.warn({}, 'Service ranking fetch timeout, returning empty array');
            resolve([]);
      ...
      if (response && response.length > 0) {
        logger.info({ rankingCount: response.length }, 'Service ranking fetched successfully');
        return response;
      }

      // 如果超时或返回空，返回空数组
      logger.warn({}, 'Service ranking fetch returned empty or timed out');
      return [];
    } catch (error: any) {
      logger.error({ error: error.message }, 'Service ranking fetch failed, returning empty array');
      return [];
    }
  });

  // 辅助函数：刷新服务排行
  async function refreshServiceRanking(): Promise<any[]> {
    try {
      const axios = require('axios');
      const config = loadNodeConfig();
      ...
      logger.info({ statsUrl }, 'Fetching service ranking from scheduler');
      ...
      // 更新缓存
      setCachedServiceRanking(ranking);
      setLastCacheUpdate(Date.now());
