...
export class SequentialExecutor {
  private state: SequentialExecutorState;
  private config: Required<SequentialExecutorConfig>;
  private timeoutCheckInterval?: NodeJS.Timeout;

  constructor(config: SequentialExecutorConfig = { enabled: true }) {
   ...
   * 将任务加入等待队列
   */
  private enqueueTask(task: SequentialTask): void {
   ...
   * 处理任务
   */
  private async processTask(task: SequentialTask): Promise<void> {
    // 检查是否已有任务正在处理（按服务类型）
    const processingState = this.state.processing.get(task.sessionId);
   ...
   * 获取当前状态（用于调试）
   */
  getState(): SequentialExecutorState {
    // 深拷贝currentIndex
    const currentIndex = new Map<string, Map<ServiceType, number>>();
    for (const [sessionId, sessionState] of this.state.currentIndex.entries()) {
      currentIndex.set(sessionId, new Map(sessionState));
    }

    // 深拷贝waitingQueue
    const waitingQueue = new Map<string, Map<ServiceType, SequentialTask[]>>();
    for (const [sessionId, sessionQueues] of this.state.waitingQueue.entries()) {
      const queues = new Map<ServiceType, SequentialTask[]>();
      for (const [taskType, queue] of sessionQueues.entries()) {
    ...
    // 深拷贝processing
    const processing = new Map<string, Map<ServiceType, SequentialTask | null>>();
   ...
   * @param reason 取消原因
   */
  cancelTask(sessionId: string, utteranceIndex: number, reason: string = 'Task merged', taskType?: ServiceType): void {
    // 从等待队列中移除（按服务类型）
    const sessionQueues = this.state.waitingQueue.get(sessionId);
    if (sessionQueues) {
      const taskTypesToCheck = taskType ? [taskType] : ['ASR', 'NMT', 'TTS', 'SEMANTIC_REPAIR'] as ServiceType[];
      
      for (const type of taskTypesToCheck) {
        const queue = sessionQueues.get(type);
        if (queue) {
          // 查找所有匹配的任务
          const tasksToCancel: SequentialTask[] = [];
  ...
  clearSession(sessionId: string): void {
    ...
    logger.info(
      { sessionId },
      'SequentialExecutor: Session state cleared'
    );
  }
  ...
  destroy(): void {
    ...
    logger.info({}, 'SequentialExecutor: Destroyed');
  }
}
