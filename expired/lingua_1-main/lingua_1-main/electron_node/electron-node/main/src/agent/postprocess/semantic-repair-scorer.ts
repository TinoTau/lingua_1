...
export interface SemanticRepairScore {
  score: number;  // 综合评分（0-1，越高越需要修复）
  reasonCodes: string[];  // 触发原因代码
  details: {
    qualityScore?: number;
    qualityScoreWeight?: number;
    shortSentenceScore?: number;
    shortSentenceWeight?: number;
    nonChineseRatio?: number;
    nonChineseRatioWeight?: number;
    syntaxScore?: number;
    syntaxWeight?: number;
    languageProbability?: number;
    languageProbabilityWeight?: number;
  };
}

export interface SemanticRepairScorerConfig {
  // 阈值配置
  qualityThreshold?: number;  // 质量分阈值（默认0.70）
  shortSentenceLength?: number;  // 短句长度阈值（默认16）
  nonChineseRatioThreshold?: number;  // 非中文比例阈值（默认0.3）
  languageProbabilityThreshold?: number;  // 语言概率阈值（默认0.7）
  
  // 权重配置
  qualityScoreWeight?: number;  // 质量分权重（默认0.4）
  shortSentenceWeight?: number;  // 短句权重（默认0.2）
  nonChineseRatioWeight?: number;  // 非中文比例权重（默认0.2）
  syntaxWeight?: number;  // 句法权重（默认0.1）
  languageProbabilityWeight?: number;  // 语言概率权重（默认0.1）
  
  // 触发阈值
  triggerThreshold?: number;  // 综合评分触发阈值（默认0.3，降低以提高敏感度）
}

export class SemanticRepairScorer {
  private config: Required<SemanticRepairScorerConfig>;

  constructor(config: SemanticRepairScorerConfig = {}) {
    this.config = {
    ...
    if (Math.abs(totalWeight - 1.0) > 0.01) {
      logger.warn(
        { totalWeight, config: this.config },
        'SemanticRepairScorer: Weights do not sum to 1.0, normalizing'
      );
      // 归一化权重
  ...
  score(
    ...
    const details: SemanticRepairScore['details'] = {};
    ...
    // 计算综合评分（加权平均）
    const totalScore = 
      qualityScoreValue * this.config.qualityScoreWeight +
   ...
   * 判断是否应该触发修复
   */
  shouldTrigger(score: SemanticRepairScore): boolean {
   ...
   * 统计非中文字符比例
   */
  private countNonChineseChars(text: string): number {
   ...
   * P1-4: 增强异常句法检测
   */
  private hasBasicSyntax(text: string): boolean {
    // 1. 检查是否包含常见动词
    const commonVerbs = [
   ...
   * P1-4: 检测垃圾字符
   */
  private hasGarbageChars(text: string): boolean {
    // 检测连续重复字符（如"啊啊啊"、"哈哈哈"）
   ...
   * P1-4: 检测异常词形（中文）
   */
  private hasAbnormalWordForm(text: string): boolean {
    // 检测连续重复字符（如"云云云"、"修修复"）
