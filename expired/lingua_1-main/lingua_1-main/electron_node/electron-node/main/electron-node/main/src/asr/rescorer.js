...
class Rescorer {
    ...
    computeRuleScore(text, ctx) {
        ...
        // 5. 长度合理性：过短或不完整扣分
        const isCjk = (0, aggregator_decision_1.looksLikeCjk)(trimmed);
        if (isCjk) {
            const chars = (0, aggregator_decision_1.countCjkChars)(trimmed);
            if (chars < 2) {
                score -= 3.0; // 过短
            }
            ...
            const words = (0, aggregator_decision_1.countWords)(trimmed);
            if (words < 2) {
                score -= 3.0; // 过短
            }
