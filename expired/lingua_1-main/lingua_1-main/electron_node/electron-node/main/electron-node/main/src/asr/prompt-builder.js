...
class PromptBuilder {
    constructor(mode = 'offline', config) {
    ...
    build(ctx) {
        try {
            ...
            logger_1.default.error({ error }, 'PromptBuilder.build failed, returning null');
     ...
     * 提取关键词
     */
    extractKeywords(ctx, includeRecent) {
     ...
     * 提取最近上下文行（带质量检查和错误过滤）
     */
    extractRecentLinesWithQualityCheck(ctx) {
     ...
     */
    compressPrompt(prompt, keywords, recentLines) {
