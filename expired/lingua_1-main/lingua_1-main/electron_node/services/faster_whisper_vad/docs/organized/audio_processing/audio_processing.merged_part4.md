...
1. **`audio_chunk`消息**（流式发送）
   - **用途**: 实时流式传输音频数据
   - **发送时机**: 录音过程中持续发送（每100ms）
   - **处理方式**: 调度服务器使用`audio_buffer`累积，在finalize时合并

2. **`utterance`消息**（一次性发送）
   - **用途**: 一次性发送完整音频
   - **发送时机**: 用户手动触发（`sendCurrentUtterance()`）
...
**优势**：
1. **实时性**: 支持流式传输，可以实时处理音频
2. **低延迟**: 不需要等待完整音频，可以边录边处理
3. **流式ASR**: 支持部分结果输出（`asr_partial`消息）
4. **自动切句**: 调度服务器可以根据暂停时间自动切句
...
**`audio_chunk`使用**：
```typescript
// app.ts 第246行：录音过程中自动发送
if (this.audioBuffer.length >= 10) {
  const chunk = this.concatAudioBuffers(this.audioBuffer.splice(0, 10));
  this.wsClient.sendAudioChunk(chunk, false);  // 流式发送
}

// app.ts 第263行：静音检测后发送剩余数据
...
async sendCurrentUtterance(): Promise<void> {
  if (this.audioBuffer.length > 0) {
    ...
    await this.wsClient.sendUtterance(...);  // 一次性发送
  }
}
```
...
**优势**：
1. **更简单**: 不需要`audio_buffer`累积逻辑
2. **更直接**: 一次性发送，直接创建job

**劣势**：
1. **失去流式能力**: 无法支持实时流式传输
2. **失去自动切句**: 需要Web端自己控制切句
...
**理由**：
1. **保留灵活性**: 支持流式和一次性发送
2. **统一处理**: 所有音频都经过相同的处理流程
3. **简化代码**: 只需要维护一套逻辑
4. **向后兼容**: 可以保留`utterance`作为兼容接口，内部转换为`audio_chunk`

**实现步骤**：
1. **Web端**: 修改`sendCurrentUtterance()`，使用`sendAudioChunk(audioData, true)`
2. **调度服务器**: 修改`handle_utterance()`，将数据添加到`audio_buffer`并finalize
3. **测试**: 确保两种方式都能正常工作
